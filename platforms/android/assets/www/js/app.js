	...
	//Constantes et fonctions globales sur #friends 
		...
		//Gestion du back-end sur #friends
			...
			//Fonction d'insertion dans #friends
						...
						try {
							...
							function chooseFriend(friendNumber) {
								TABfriends_result[friendNumber].addEventListener("click",function() {
									...
									if(inJSONID(currentUser.friendsIDser,friends_profPage.currUser.ID)) friends_profPage.isFriend = true;
									else if(friends_profPage.currUser.ID == currentUser.ID) friends_profPage.isFriend = "same";
									else if(inJSONID(currentUser.friendsReqIDser,friends_profPage.currUser.ID)) friends_profPage.isFriend = "waiting";
									else if(inJSONID(friends_profPage.currUser.friendsReqIDser,currentUser.ID)) friends_profPage.isFriend = "accept";
									else friends_profPage.isFriend = false;
									insertInFriendsProfPage(friendNumber);
								},false);
							}
							chooseFriend(i); 
						} 
						catch(err){console.log("Erreur d'insertion des proofs")}
					}
				}
				function insertInFriendsProfPage(friendNumber) { 
					...
					if(friends_profPage.isFriend == true) {
						friends_profPage_beFriend.style.visibility = "visible";
						friends_profPage_beFriend.querySelector("i").setAttribute("class","fa fa-check");
						friends_profPage_beFriend_situation.innerHTML = " Ami";
					} 
					else if(friends_profPage.isFriend == "same") {
						friends_profPage_beFriend.style.visibility = "hidden";
					} 
					else if(friends_profPage.isFriend == "accept") {
						friends_profPage_beFriend.style.visibility = "visible";
						friends_profPage_beFriend.querySelector("i").setAttribute("class","fa fa-question");
						friends_profPage_beFriend_situation.innerHTML = " Accepter";
					} 
					else if(friends_profPage.isFriend == "waiting") {
						friends_profPage_beFriend.style.visibility = "visible";
						friends_profPage_beFriend.querySelector("i").setAttribute("class","fa fa-hourglass-o");
						friends_profPage_beFriend_situation.innerHTML = " En attente";
					} 
					else {
						friends_profPage_beFriend.style.visibility = "visible";
						friends_profPage_beFriend.querySelector("i").setAttribute("class","fa fa-plus");
						friends_profPage_beFriend_situation.innerHTML = " Ajouter";						
					}
				}
			...
			//Objet AjaxFunction pour ajouter un ami
				var addFriend = new AjaxFunction("GET","http://www.crabbix.fr/addFriend.php",["userID","friendID"],10000,{
